  ...
  <div class="min-h-screen flex items-center justify-center bg-beige-light px-4">
    <div class="max-w-md w-full glass p-8 rounded-2xl shadow-2xl border border-brand-gold/20">
      <div class="text-center mb-10">
        <h1 class="text-3xl font-playfair font-bold text-brand-navy mb-2 tracking-widest">CREATE ACCOUNT</h1>
        <p class="text-xs text-brand-purple uppercase tracking-[0.2em] font-medium">Join our exclusive studio</p>
      </div>

      <form @submit.prevent="handleRegister" class="space-y-6">
        <div>
          <label class="block text-[10px] uppercase tracking-widest text-brand-navy font-bold mb-2 ml-1">Full Name</label>
          <input 
            v-model="form.name"
            type="text" 
            required
        ...
        <button 
          type="submit" 
          :disabled="loading"
          class="w-full bg-brand-navy text-white font-bold py-4 rounded-xl uppercase tracking-[0.3em] text-[10px] hover:bg-brand-rose hover:shadow-lg transition-all transform hover:-translate-y-1 active:scale-95 disabled:opacity-50"
        >
      ...
      <div class="mt-10 text-center">
        <p class="text-[10px] uppercase tracking-widest text-brand-navy/60 font-medium">
          ...
          <router-link to="/login" class="text-brand-purple font-bold hover:text-brand-gold transition-colors ml-1 underline decoration-brand-gold/30 underline-offset-4">Sign In</router-link>
        </p>
      </div>
    </div>
  </div>
